...
const MediaDashboard = () => {
  ...
  const [isIndividualInterchange, setIsIndividualInterchange] = useState(false)
    ...
    return (
      <div className="overflow-x-auto overflow-y-auto flex-1 table-custom-scrollbar">
        <table className="w-full text-left">
          ...
          <tbody>
            {console.log("individual req media : ", individualRequests)}
            {individualRequests.map((item) => {
              const data = item.data || {}
              const emp = data?.employeeDetail?.firstName || item.employeeDetail || {}
              const requestNo = data.requestNo || item.requestNo || '-'
              const empName = item.employeeDetail?.firstName || emp.name || '-'
                    ...
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          const media = Array.isArray(data.mediaRequirements)
                            ? data.mediaRequirements
                            : Array.isArray(data.media)
                              ? data.media
                              : [data]
                          const types = deriveAvailableMediaTypes(media)

                          if (!submissionId || types.length === 0) return

                          setSelectedEventForInterchange({
                            eventId: submissionId,
                            _id: submissionId,
                            media,
                          })
                          setInterchangeMediaType(types[0])
                          setIsIndividualInterchange(true)
                        }}
                        className="flex h-8 w-8 items-center justify-center text-[#8b93a7] transition hover:text-white"
                        title="Request to interchange"
                        aria-label={`Request interchange for ${requestNo}`}
                      >
                        <Shuffle size={17} />
                      </button>
                    </div>
    ...
    <>
      ...
      {selectedEventForInterchange && interchangeMediaType && (
        <MediaStaffInterchangeModal
          ...
          isIndividualInterchange={isIndividualInterchange}
          onClose={() => {
            setSelectedEventForInterchange(null)
            setInterchangeMediaType('')
            setIsIndividualInterchange(false)
          }}
          onSuccess={isIndividualInterchange ? fetchIndividualRequests : fetchMediaDashboardData}
        />
      )}
    </>
  )
