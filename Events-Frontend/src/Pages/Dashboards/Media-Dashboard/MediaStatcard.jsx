...
const MediaStatcard = ({ sections = defaultSections }) => {
    ...
    useEffect(() => {
        ...
        fetch(`${API_BASE_URL}/api/dashboard/stats?module=media`, {
            headers: token ? { Authorization: `Bearer ${token}` } : {},
        })
            .then((response) => {
                if (!response.ok) {
                    throw new Error('Failed to fetch media dashboard stats')
                }
                return response.json()
            })
            .then((responseData) => {
                if (isMounted) {
                    setEventStats(responseData.modules?.media ?? responseData.events ?? null)
                }
            })
            .catch((error) => {
        ...
        fetch(`${API_BASE_URL}/api/dashboard/individual-stats?module=media`, {
            headers: token ? { Authorization: `Bearer ${token}` } : {},
        })
            .then((response) => {
                if (!response.ok) {
                    throw new Error('Failed to fetch individual media dashboard stats')
                }
                return response.json()
            })
            .then((responseData) => {
