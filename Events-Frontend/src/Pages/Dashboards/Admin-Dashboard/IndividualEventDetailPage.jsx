...
import { ChevronRight, FileText, Check, X, Shuffle } from "lucide-react";
...
const IndividualEventDetailPage = () => {
  ...
  const [isMediaInterchangeOpen, setIsMediaInterchangeOpen] = useState(false);
  ...
  const isMediaRequest = formTypeKey === "media";
  const mediaRequirements = Array.isArray(data.mediaRequirements)
  ...
  return (
    <section className="min-h-screen bg-[#0b1326] poppins">
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  {isMediaRequest && (
                    <button
                      type="button"
                      onClick={() => setIsMediaInterchangeOpen(true)}
                      className="hidden items-center gap-2 rounded-lg bg-[#8B3DFF]/15 px-4 py-2 text-sm font-medium text-[#D0BCFF] transition hover:bg-[#8B3DFF]/25"
                    >
                      <Shuffle size={16} />
      ...
      {isMediaInterchangeOpen && isMediaRequest && (
        <MediaStaffInterchangeModal
          event={{ eventId: id, _id: id, media: mediaRequirements }}
          onClose={() => setIsMediaInterchangeOpen(false)}
