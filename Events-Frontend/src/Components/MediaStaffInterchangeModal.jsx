...
const normalizeMediaType = (type) => {
  ...
  if (value.includes('poster')) types.push('poster')
  if (value.includes('video')) types.push('video')
...
const deriveAvailableMediaTypes = (media = []) => [
  ...new Set(
    media.flatMap((item) => {
      const rawTypes = Array.isArray(item?.typeOfMedia)
        ? item.typeOfMedia
        : [item?.typeOfMedia].filter(Boolean)

      return rawTypes.flatMap(normalizeMediaType)
    })
  ),
]
...
const MediaStaffInterchangeModal = ({ event, mediaType: initialMediaType, onClose, onSuccess, isIndividualInterchange = false, title: customTitle }) => {
  ...
  console.log("media type : ", initialMediaType)
          ...
          return (
            <li key={faculty.email}>
              <button
                ...
                disabled={isSubmitting}
                className={`flex w-full items-center gap-3 px-4 py-3 text-left transition disabled:cursor-not-allowed ${isSelected
                ...
                {['poster', 'video'].map((type) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isSubmitting}
                      className={`flex items-center gap-2 rounded-lg border px-4 py-2.5 text-sm font-medium transition disabled:cursor-not-allowed ${isSelected
                        ? 'border-[#8B3DFF] bg-[#8B3DFF]/15 text-white'
                        : 'border-[#283247] text-[#CBC3D7] hover:border-[#8B3DFF]/50 hover:text-white'
                        }`}
                    >
                      <span
                        className={`flex h-4 w-4 shrink-0 items-center justify-center rounded-full border-2 transition ${isSelected
                          ? 'border-[#8B3DFF] bg-[#8B3DFF]'
                          : 'border-[#4a5270]'
                          }`}
                      >
