...
const PurchaseEventsDetailViewPage = () => {
  ...
  const handleStatusUpdate = async (action) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/events/${eventId}/status`, {
        ...
        body: JSON.stringify({ action, module: 'purchase' }),
      })
      ...
      <main className="h-[93vh] px-7 pt-2">
        <header className="mt-4 flex items-center justify-between gap-5">
          <div className="flex items-center gap-2">
            <Link to="/dashboard-purchase" className="text-md font-medium text-[#CBC3D7]/50 transition hover:text-white">Event Details</Link>
          ...
          <section className="max-h-[calc(100vh-170px)] overflow-auto rounded-lg border border-[#27334c] bg-[#151d31] p-5 table-custom-scrollbar">
            {loading ? (
              ...
              <>
                ...
                <div>
                  <h2 className="text-lg font-medium text-[#8B3DFF]">Purchase Details</h2>
                  <p className="mt-2 text-xs leading-6 text-[#CBC3D7]/55">
                    Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry&apos;s standard dummy text ever since the 1500s
                  </p>
                </div>
                {status && <span className={`rounded-full px-5 py-2 whitespace-nowrap text-sm font-medium ${getStatusClassName(status)}`}>{status}</span>}
              </div>
...
export default PurchaseEventsDetailViewPage
