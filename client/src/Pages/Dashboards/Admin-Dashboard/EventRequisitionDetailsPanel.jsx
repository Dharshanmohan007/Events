        ...
        {items.map(([label, value, boolean]) => (
            <div key={label} className="flex items-center justify-between border-b border-[#30384d]/60 py-3 text-sm last:border-b-0">
                ...
                {typeof boolean === 'boolean' ? (
                    <YesNoValue value={value} positive={boolean} />
                ) : (
                    <span className="font-medium text-[#E6E2F0]">{displayValue(value)}</span>
                )}
            </div>
        ))}
    </div>
...
const DocumentLink = ({ document, label }) => {
    ...
    return (
        <a href={document.url} target="_blank" rel="noreferrer" className="flex items-center gap-3 pl-8 text-sm font-medium text-white hover:text-[#c6b5ff]">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#0e5149]/30 text-[#0fc982]">
                <FileText size={17} />
            </span>
            <span>{label}</span>
        </a>
...
const EventRequisitionDetailsPanel = ({ requestDetails }) => {
    ...
    if (!requestDetails) {
        return (
            <div className="space-y-5">
                <p className="py-10 text-center text-sm text-[#CBC3D7]/65">No event requisition details are available.</p>
            </div>
        )
    }
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="grid grid-cols-[1fr_1fr] items-center rounded-lg border border-[#374155] bg-[#232A3B] px-4 py-4">
                ...
                {organizer.previousEventDocumentation ? (
                    <DocumentLink document={organizer.previousEventDocumentationDetails} label="Previous Event Completion Document" />
                ) : (
                    <p className="pl-8 text-sm text-white">{displayValue(organizer.previousEventReason)}</p>
                )}
            </div>
            ...
            <section className="space-y-3 rounded-lg border border-[#374155] bg-[#2E3545] p-4">
                {(organizer.organizers || []).length > 0 ? (
                    ...
                    <p className="py-2 text-center text-sm text-[#CBC3D7]/65">No organizer details available.</p>
                )}
            </section>
            ...
            <section className="rounded-lg border border-[#374155] bg-[#232A3C] p-5">
                ...
                {organizer.financeRequired && organizer.purposeOfAdvance && (
                    <p className="mt-4 border-t border-[#30384d]/60 pt-4 text-sm text-[#CBC3D7]/75">
                        Purpose of advance: <span className="text-white">{organizer.purposeOfAdvance}</span>
                    </p>
                )}
            </section>
            ...
            <section className="space-y-3 rounded-lg border border-[#374155] bg-[#2E3545] p-4">
                {(currentDay.guests || []).length > 0 ? (
                    (currentDay.guests || []).map((guest, index) => (
                        <InfoRow
                            key={`${guest.mobile || guest.name}-${index}`}
                            items={[
                    ...
                    <p className="py-2 text-center text-sm text-[#CBC3D7]/65">No guests scheduled for this day.</p>
                )}
            </section>
        </div>
    )
