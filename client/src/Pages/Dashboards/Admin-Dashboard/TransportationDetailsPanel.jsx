...
const formatDateTime = (value, options) => (value ? new Intl.DateTimeFormat('en-GB', options).format(new Date(value)) : '-')
...
const TransportTimeline = ({ transport }) => {
  const checkpoints = transport.checkpoints?.map((c) => c.location).filter(Boolean).join(', ') || '-'
  ...
  <div className="grid grid-cols-2 rounded-md border border-[#374155]/70 bg-[#242B3D] px-4 py-4">
    {items.map(([label, value, Icon], index) => (
      <div key={label} className={`flex items-center justify-between gap-5 px-2 text-sm ${index === 0 ? 'border-r border-[#6b7280]/50 pr-9' : 'pl-5'}`}>
        {Icon ? <IconInfoCell icon={Icon} label={label} value={value} /> : <><span className="text-[#CBC3D7]/75">{label}</span><span className="font-semibold text-white">{value}</span></>}
      </div>
    ))}
  </div>
)
...
const TransportDetails = ({ transport }) => {
  ...
  const transportSummary = [
    { icon: CalendarDays, label: 'Pickup Date', value: formatDateTime(transport.pickupDateTime, { day: '2-digit', month: '2-digit', year: 'numeric' }) },
    { icon: Clock3, label: 'Pickup Time', value: formatDateTime(transport.pickupDateTime, { hour: '2-digit', minute: '2-digit', hour12: true }) },
    { icon: CalendarDays, label: 'Drop Date', value: formatDateTime(transport.dropDateTime, { day: '2-digit', month: '2-digit', year: 'numeric' }) },
  ...
  return (
    <div className="space-y-5 rounded-lg border border-[#374155] bg-[#1B2334] p-4">
      <div className="grid grid-cols-4 rounded-md border border-[#374155]/60 bg-[#242B3D] py-4">
        {transportSummary.map((item, index) => <IconInfoCell key={item.label} {...item} className={`px-4 ${index !== transportSummary.length - 1 ? 'border-r border-[#6b7280]/50' : ''}`} />)}
      </div>
      ...
      {transport.specialRequirements ? (
        <section className="rounded-lg border border-[#465168] bg-[#232A3B] p-5">
          ...
          <p className="text-sm font-medium leading-7 text-[#E6E2F0]">{displayValue(transport.specialRequirements)}</p>
        </section>
      ) : null}
    </div>
  )
...
const TransportationDetailsPanel = ({ transportDetails, eventSchedule = [] }) => {
  const [activeDay, setActiveDay] = useState(0)
  const transports = transportDetails?.transports || []
  if (!transportDetails) return <p className="py-10 text-center text-sm text-[#CBC3D7]/65">No transportation details are available.</p>
  ...
  return (
    <div className="space-y-5">
      {dayCount > 1 && (
        <nav className="flex border-b border-[#374155]" aria-label="Transportation event days">
      ...
      {dayTransport ? <TransportDetails transport={dayTransport} /> : <p className="py-8 text-center text-sm text-[#CBC3D7]/65">No transportation details were submitted for Day {selectedDay + 1}.</p>}
    </div>
  )
}

export default TransportationDetailsPanel
