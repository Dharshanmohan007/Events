...
const SplitInfoRow = ({ items }) => (
  <div className="grid grid-cols-3 rounded-md border border-[#374155]/60 bg-[#242B3D] px-4 py-4">
...
const MealSection = ({ title, foodType, showPlacement = false }) => {
  ...
  const isRefreshment =
    foodType.type === "Morning Refreshment" ||
    foodType.type === "Evening Refreshment";
        ...
        {isRefreshment ? (
          <>
            <div className="rounded-md border border-[#374155]/60 bg-[#242B3D] px-4 py-4">
              ...
              <p className="mt-1 text-sm font-semibold text-white">
                {displayValue(refreshmentCount)}
              </p>
            </div>
            {venueWiseDetails.length > 0 && (
              <div className="rounded-md border border-[#374155]/60 bg-[#242B3D] px-4 py-4">
                <p className="text-[10px] font-semibold uppercase text-[#CBC3D7]/45 mb-3">
                  Venue Wise Details
                </p>
                  ...
                  {venueWiseDetails.map((venue, index) => (
                    ...
                    >
                      <span className="text-white capitalize">
                        {displayValue(venue.venueName)}
                      </span>
                      <span className="text-[#CBC3D7]/70">
                        {displayValue(venue.count)}
                      </span>
                    </div>
                  ))}
                </div>
          ...
          <>
            <SplitInfoRow
              items={[
                ...
                [
                  ...
                  displayValue(vipGuests.vegCount),
                ],
              ]}
            />
            <div className="rounded-md border border-[#374155]/60 bg-[#242B3D] px-4 py-4">
              <p className="text-[10px] font-semibold uppercase text-[#CBC3D7]/45">
                No. of veg In Trainer Menu
              </p>
              <p className="mt-1 text-sm font-semibold text-white">
                {displayValue(trainer.vegCount)}
              </p>
            </div>
            {showPlacement && (
              <div className="rounded-md border border-[#374155]/60 bg-[#242B3D] px-4 py-4">
                <p className="text-[10px] font-semibold uppercase text-[#CBC3D7]/45">
                  No. of veg In Placement Menu
                </p>
                <p className="mt-1 text-sm font-semibold text-white">
                  {displayValue(placement.vegCount)}
                </p>
              </div>
            )}
            ...
            {((foodType.type !== "Breakfast" && foodType.type !== "Dinner") ||
              ...
              <>
                ...
                <div className="rounded-md border border-[#374155]/60 bg-[#242B3D] px-4 py-4">
                  <p className="text-[10px] font-semibold uppercase text-[#CBC3D7]/45">
                    No. of Non-veg In Trainer Menu
                  </p>
                  <p className="mt-1 text-sm font-semibold text-white">
                    {displayValue(trainer.nonVegCount)}
                  </p>
                </div>
                {showPlacement && (
                  <div className="rounded-md border border-[#374155]/60 bg-[#242B3D] px-4 py-4">
                    <p className="text-[10px] font-semibold uppercase text-[#CBC3D7]/45">
                      No. of Non-veg In Placement Menu
                    </p>
                    <p className="mt-1 text-sm font-semibold text-white">
                      {displayValue(placement.nonVegCount)}
                    </p>
                  </div>
                )}
              </>
...
const FoodDetails = ({ refreshment }) => {
  console.log("Food from date admin : ", refreshment);
  ...
  const summaryRows = [
    [
      ["From date", formatDate(refreshment.fromDate), CalendarDays],
      ["To date", formatDate(refreshment.toDate), CalendarDays],
    ...
    <div className="space-y-5">
      ...
      {foodTypes.map((foodType) => (
        <MealSection
          ...
          foodType={foodType}
          showPlacement={(refreshment.resourcePersonType || []).includes(
            "Placement",
          )}
        />
      ))}
