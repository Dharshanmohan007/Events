...
const getFilteredDateValues = (values, dateKeys, selectedDateKey) => {
  ...
  const selectedDateIndex = dateKeys.findIndex(
...
export default function AdminEventsRequestTable() {
  ...
  useEffect(() => {
    ...
    fetch(EVENT_REQUEST_URL, {
      ...
      .then((responseData) => {
        if (isMounted)
          setEventRows((responseData.data || []).map(normalizeEventRequest));
      })
      .catch((error) => {
  ...
  const eventTypeOptions = useMemo(() => {
    const eventTypes = [
      ...new Set(eventRows.map((event) => event.eventType).filter(Boolean)),
    ];
    return [
  ...
  const filteredRows = activeRows.filter((row) => {
    ...
    const searchableText = Object.values(row).flat().join(" ").toLowerCase();
        ...
        rows.map((event, index) => (
          ...
          >
            ...
            <td className="px-6 py-4 whitespace-nowrap">
              <span
                className={`${event.isClosed ? "text-green-500" : "text-amber-500"}`}
              >
                {event.isClosed ? "Closed" : "Not closed"}
              </span>
            </td>
            <td className="px-6 py-4 whitespace-nowrap">
