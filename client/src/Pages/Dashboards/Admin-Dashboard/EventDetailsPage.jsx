...
const EventDetailsPage = () => {
  ...
  const renderActivePanel = () => {
    ...
    if (activeTab === 'Venue Details') {
      if (venueLoading) return <p className="py-10 text-center text-sm text-[#CBC3D7]/65">Loading venue details...</p>
      if (venueError) return <p className="py-10 text-center text-sm text-[#FF4F91]">{venueError}</p>
      return <VenueDetailsPanel venueDetails={venueDetails} eventSchedule={requestDetails?.eventDetails?.eventSchedule} />
    }

    if (activeTab === 'ICTCS Details') {
      if (ictsLoading) return <p className="py-10 text-center text-sm text-[#CBC3D7]/65">Loading ICTS details...</p>
      if (ictsError) return <p className="py-10 text-center text-sm text-[#FF4F91]">{ictsError}</p>
      return <IctcsDetailsPanel ictsDetails={ictsDetails} eventSchedule={requestDetails?.eventDetails?.eventSchedule} />
    }

    if (activeTab === 'Audio Details') {
      if (audioLoading) return <p className="py-10 text-center text-sm text-[#CBC3D7]/65">Loading audio details...</p>
      if (audioError) return <p className="py-10 text-center text-sm text-[#FF4F91]">{audioError}</p>
      return <AudioDetailsPanel audioDetails={audioDetails} eventSchedule={requestDetails?.eventDetails?.eventSchedule} />
    }
    ...
    if (activeTab === 'Accommodation Details') {
      if (accommodationLoading) return <p className="py-10 text-center text-sm text-[#CBC3D7]/65">Loading accommodation details...</p>
      if (accommodationError) return <p className="py-10 text-center text-sm text-[#FF4F91]">{accommodationError}</p>
      return <AccommodationDetailsPanel accommodationDetails={accommodationDetails} eventSchedule={requestDetails?.eventDetails?.eventSchedule} />
    }

    if (activeTab === 'Purchase Details') {
      if (purchaseLoading) return <p className="py-10 text-center text-sm text-[#CBC3D7]/65">Loading purchase details...</p>
      if (purchaseError) return <p className="py-10 text-center text-sm text-[#FF4F91]">{purchaseError}</p>
      return <PurchaseDetailsPanel purchaseDetails={purchaseDetails} eventSchedule={requestDetails?.eventDetails?.eventSchedule} />
    }

    if (activeTab === 'Media Details') {
      if (mediaLoading) return <p className="py-10 text-center text-sm text-[#CBC3D7]/65">Loading media details...</p>
      if (mediaError) return <p className="py-10 text-center text-sm text-[#FF4F91]">{mediaError}</p>
      ...
      <main className="px-6 pb-8 ">
        ...
        <section className="mt-2 flex min-h-[calc(100vh-160px)] gap-2">
          ...
          <main className="flex-1 overflow-auto rounded-lg border border-[#27334c] bg-[#151d31] p-5 table-custom-scrollbar max-h-[calc(100vh-130px)]">
            {requisitionLoading ? (
              <p className="py-10 text-center text-sm text-[#CBC3D7]/65">Loading event details...</p>
            ) : requisitionError ? (
              ...
              <p className="py-10 text-center text-sm text-[#CBC3D7]/65">No details available for this event.</p>
            )}
          </main>
        </section>
      </main>
