...
const AudioEventsDetailViewPage = () => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      try {
        ...
        setError(err.message || 'Failed to fetch audio details')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleStatusUpdate = async (action) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/events/${eventId}/status`, {
        ...
        body: JSON.stringify({ action, module: 'audio' }),
      })
    ...
    <section className="min-h-screen bg-[#0b1326] poppins">
      ...
      <main className="px-6 pb-8">
        <div className="flex items-center gap-2 py-3 text-sm text-[#CBC3D7]/50">
          <Link to="/dashboard-audio" className="hover:text-white transition-colors">
            Audio Dashboard
          </Link>
          <ChevronRight size={14} />
          <span className="text-[#D0BCFF]">{eventName || 'Audio Details'}</span>
        </div>
          ...
          {loading ? (
            ...
            <>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h2 className="text-lg font-medium text-[#8B3DFF]">Audio Details</h2>
...
export default AudioEventsDetailViewPage
