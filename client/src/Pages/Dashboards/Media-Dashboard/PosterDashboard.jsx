...
export const derivePosterStatus = (posterRequirements = []) => {
  ...
  useEffect(() => {
    ...
    const fetchPosterEvents = async () => {
      ...
      try {
        ...
        const url = `${API_BASE_URL}/api/dashboard/poster-dashboard?email=${encodeURIComponent(email)}`
        ...
        if (json.events && Array.isArray(json.events)) {
          setEvents(transformPosterEvents(json.events))
        } else {
          setEvents([])
        }
      } catch (err) {
        if (err.name === 'AbortError') return
        console.error('Failed to fetch poster dashboard data:', err)
        setError(err.message || 'Failed to load poster requests')
      } finally {
    ...
    if (events.length === 0) {
      return (
        <div className="flex flex-1 items-center justify-center">
          <p className="text-sm text-[#CBC3D7]/65">No poster event requests found.</p>
        </div>
      )
    }
              ...
              return (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-center">
                    <Link
                      to={`/dashboard-poster/detailView/${event._id}`}
