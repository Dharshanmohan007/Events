  ...
  useEffect(() => {
    ...
    const fetchSubmission = async () => {
      ...
      try {
        ...
        if (!res.ok) throw new Error("Failed to fetch food submission");
        const response = await res.json();
        if (!response.success)
          throw new Error(response.message || "Failed to fetch food submission");
        if (isMounted) {
              ...
              <>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-xs leading-6 text-[#CBC3D7]/55">
                      Complete details of the individual food request
                      submission.
                    </p>
                  </div>
                  {status && (
                ...
                <div className="mt-8">
                  <FacultyFoodRefreshmentDetailsPanel
                    refreshmentDetails={{ refreshments: [innerData] }}
                    eventSchedule={[]}
                  />
                </div>
