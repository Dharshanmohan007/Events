    ...
    {items.map(([label, value, Icon], index) => (
      <div key={label} className={`flex items-center justify-between gap-5 px-2 text-sm ${index === 0 ? 'border-r border-[#6b7280]/50 pr-9' : 'pl-5'}`}>
        <div className="flex items-start gap-3">
          {Icon && <Icon size={16} className="mt-0.5 shrink-0 text-[#C9B6FF]" />}
          <div>
...
const FoodDetails = ({ refreshment }) => {
  const resourcePersonTypes = (refreshment.resourcePersonType || []).filter(Boolean).join(' / ') || '-'
  ...
  const summaryRows = [
    ...
    [
      ['Total number of Resource Persons', displayValue(refreshment.numberOfResourcePersons)],
      ['Total number of Internal Accompanying Staff', displayValue(refreshment.numberOfInternalAccompanyingStaff)],
    ],
    [
    ...
    <div className="space-y-5">
      ...
      {refreshment.specialRequirements ? (
        <section className="rounded-lg border border-[#465168] bg-[#232A3B] p-5">
          ...
          <p className="text-sm font-medium leading-7 text-[#E6E2F0]">{displayValue(refreshment.specialRequirements)}</p>
        </section>
      ) : null}
    </div>
  )
...
const FacultyFoodRefreshmentDetailsPanel = ({ refreshmentDetails, eventSchedule = [] }) => {
  ...
  return (
    <div className="space-y-5">
      {dayCount > 1 && (
        <nav className="flex border-b border-[#374155]" aria-label="Food event days">
      ...
      {dayRefreshment ? (
        ...
        <p className="py-8 text-center text-sm text-[#CBC3D7]/65">No food details were submitted for Day {selectedDay + 1}.</p>
      )}
    </div>
  )
}
