...
const Status = ({ status }) => {
  ...
  return (
    <span
      className={`flex items-center gap-1.5 text-[10px] font-semibold ${isApproved ? "text-[#20D18C]" : "text-[#F20768]"}`}
    >
      <span
        className={`h-1.5 w-1.5 rounded-full ${isApproved ? "bg-[#20D18C]" : "bg-[#F20768]"}`}
      />
      {status}
    </span>
  );
...
const FacultyLatestEventsRequestTable = () => {
  ...
  useEffect(() => {
    const fetchIndividualRequests = async () => {
      try {
        ...
        const decoded = jwtDecode(token);
        const facultyId =
          decoded.facultyId || decoded.id || decoded._id || decoded.userId;
  ...
  useEffect(() => {
    ...
    if (query) {
      result = result.filter((e) => {
        ...
        return [e.eventName, e.eventType, dateStr, e.eventStatus]
          .join(" ")
          .toLowerCase()
          .includes(query);
      });
  ...
  const hasActiveFilters =
    statusFilter || eventTypeFilter || dateFrom || dateTo;
    ...
    <section className="flex max-h-[570px] flex-col overflow-hidden rounded-lg border border-[#263044] bg-[#151d2d]">
      <div className="flex flex-shrink-0 items-center justify-between px-4 py-4">
        ...
        <div className="flex items-center gap-4">
          {/* Tabs */}
          <nav
            className="flex rounded-md bg-[#1b2435] p-0.5"
            ...
            <div className="relative" ref={filterRef}>
              ...
              >
                ...
                {hasActiveFilters && (
                  <span className="ml-1 flex h-4 w-4 items-center justify-center rounded-full bg-[#F20768] text-[8px] font-bold">
                    {
                      [statusFilter, eventTypeFilter, dateFrom, dateTo].filter(
                        Boolean,
                      ).length
                    }
                  </span>
              ...
              {showFilters && (
                <div className="absolute right-0 top-full z-20 mt-2 w-64 rounded-lg border border-[#283247] bg-[#151d2e] p-4 shadow-xl">
                  <div className="mb-3 flex items-center justify-between">
                    ...
                    {hasActiveFilters && (
                      <button
                        ...
                        className="text-[10px] text-[#F20768] cursor-pointer hover:underline"
                      >
                        Clear all
                      </button>
                    )}
                  ...
                  <div className="mb-3">
                    <label className="mb-1 block text-[10px] text-[#FFFFFF80]">
                      Status
                    </label>
                    <select
                      value={statusFilter}
          ...
          <Link
            to={
              activeTab === "events"
                ? "/dashboard-faculty/events"
                : "/dashboard-faculty/individual-requests"
            }
          ...
          <table className="w-full min-w-[520px]">
            <thead className="sticky top-0 z-10 bg-[#1b2435]">
              <tr>
                {[
                  ...
                  <th
                    key={column}
                    className="px-4 py-3 text-left text-[10px] font-semibold text-[#FFFFFF66]"
                  >
                    {column}
                  </th>
                ))}
            ...
            <tbody>
              {filteredEvents.map((event) => (
                <tr
                  key={event.eventId}
                  className="border-b border-[#222b3d] last:border-b-0"
                >
                  ...
                  <td className="px-4 py-3 text-[12px] text-white">
                    {new Date(event.eventDates[0]).toLocaleDateString("en-IN", {
                      day: "2-digit",
                      month: "2-digit",
                      year: "numeric",
                    })}
                  ...
                  <td className="px-4 py-3">
                    <Status status={event.eventStatus} />
                  </td>
                  <td className="px-4 py-3">
                    <Link
              ...
              {filteredEvents.length === 0 && (
                <tr>
                  <td
                    colSpan={5}
                    className="px-4 py-6 text-center text-[12px] text-[#FFFFFF66]"
                  >
                    No events found.
                  </td>
                </tr>
              ...
              {filteredIndividualRequests.map((request) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    ...
                    <Link
                      to={`/dashboard-faculty/individualDetailView/${request.id}`}
                      className="inline-flex text-[#FFFFFF80] transition hover:text-white"
                      title="Open request"
                    >
