  ...
  <div>
    {items.map((requirement, index) => (
      ...
      >
        ...
        <span className="font-medium text-[#E6E2F0]">
          {displayValue(requirement.quantity)}
        </span>
      </div>
    ))}
  </div>
    ...
    <section className="rounded-lg border border-[#374155] bg-[#232A3B] p-5">
      ...
      <div className="grid grid-cols-2 gap-7">
        <div className="border-r border-[#6b7280]/50 pr-7">
          <RequirementColumn items={requirements.slice(0, splitAt)} />
        </div>
...
const FacultyVenueDetailsPanel = ({
  ...
  const dayCount = Math.max(
    ...
    ...venues.map((venue) => Number(venue.dayIndex) + 1),
    1,
  );
  const selectedDay = Math.min(activeDay, dayCount - 1);
  const dayVenues = venues.filter(
    (venue) => Number(venue.dayIndex) === selectedDay,
  );
  const participantCount = dayVenues.reduce(
  ...
  if (!venueDetails)
    return (
      <p className="py-10 text-center text-sm text-[#CBC3D7]/65">
        No venue details are available.
      </p>
    );
  ...
  return (
    <div className="space-y-5 ">
      ...
      {dayVenues.map((venue, index) => (
        <VenueBlock key={`${venue.venueName}-${index}`} venue={venue} />
      ))}
      {!dayVenues.length && (
        <p className="py-8 text-center text-sm text-[#CBC3D7]/65">
          No venue requirements were submitted for Day {selectedDay + 1}.
        </p>
      )}
    </div>
  );
