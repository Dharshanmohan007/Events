...
const SplitInfoRow = ({ items }) => {
  ...
  return (
    <div
      className={`${single ? "grid-cols-1" : "grid-cols-2"} grid rounded-lg border border-[#374155]/70 bg-[#242B3D] px-4 py-5`}
    >
      {items.map(([label, value], index) => (
        <div
          key={label}
          className={`flex items-center justify-between gap-5 px-2 text-sm ${!single && index === 0 ? "border-r border-[#6b7280]/50 pr-9" : !single ? "pl-5" : ""}`}
        >
...
const buildGiftRows = (data) => {
  ...
  const cashPrize = giftItems.reduce(
    (sum, g) => sum + (Number(g.cashPrizeAmount) || 0),
    0,
  );
  const glassCup = giftItems.reduce(
    (sum, g) => sum + (Number(g.glassCupQty) || 0),
    0,
  );
  const vouchers = giftItems.flatMap((g) =>
    (g.voucher || []).filter((v) => Number(v.quantity) > 0),
  );
  ...
  for (let i = 0; i < trophies.length; i += 2) {
    rows.push(
      trophies
        .slice(i, i + 2)
        .map((t) => [
          `${t.trophyType} Trophy Quantity`,
          displayValue(t.quantity),
        ]),
    );
  }
  ...
  if (glassCup > 0)
    prizeItems.push(["Glass Cup Quantity", displayValue(glassCup)]);
  if (data.registrationKitNeeded)
    prizeItems.push([
      "Registration Kit Quantity",
      displayValue(data.registrationKitQty),
    ]);
  ...
  return (
    <section className="rounded-lg border border-[#465168] bg-[#1B2334] p-5">
      ...
      <div className="mt-5 space-y-4">
        {rows.length > 0 ? (
          rows.map((row, i) => <SplitInfoRow key={i} items={row} />)
        ) : (
          <p className="py-2 text-center text-sm text-[#CBC3D7]/65">
            No items specified for {title.toLowerCase()}.
          </p>
        )}
        {data.specialRequirements && (
          <SpecialRequirement text={data.specialRequirements} />
        )}
      </div>
    </section>
  );
...
const FacultyPurchaseDetailsPanel = ({
  purchaseDetails,
  eventData,
  eventSchedule = [],
}) => {
  ...
  if (!purchaseDetails)
    return (
      <p className="py-10 text-center text-sm text-[#CBC3D7]/65">
        No purchase details are available.
      </p>
    );
    ...
    <div className="space-y-5">
      ...
      {dayCount > 1 && (
        <nav
          ...
          aria-label="Purchase event days"
        >
      ...
      {dayPurchase ? (
        <>
          ...
          {requirementRows.length > 0 && (
            <section className="rounded-lg border border-[#465168] bg-[#1B2334] p-5">
              <h3 className="text-lg font-semibold text-[#8F5BFF]">
              ...
              <div className="mt-4 space-y-3">
                {requirementRows.map((row, index) => (
                  <SplitInfoRow
                    key={index}
                    items={row.map((req) => [
                      `${req.type} Hard Copy Quantity`,
                      displayValue(req.hardCount),
                    ])}
                  />
                ))}
              </div>
          ...
          {(dayPurchase.requiredFor || []).length > 0 && (
            <SplitInfoRow
              items={[["Required For", dayPurchase.requiredFor.join(", ")]]}
            />
          )}
          ...
          <RecipientSection
            title="Students"
            data={dayPurchase.students || { giftItems: [] }}
          />

          {/* Guests Section */}
          <RecipientSection
            title="Guests"
            data={dayPurchase.guests || { giftItems: [] }}
          />
        </>
      ) : (
        <p className="py-8 text-center text-sm text-[#CBC3D7]/65">
          No purchase details were submitted for Day {selectedDay + 1}.
        </p>
      )}
    </div>
  );
