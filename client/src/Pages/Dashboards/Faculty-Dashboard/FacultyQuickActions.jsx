...
const actions = [
    { title: 'Transport Request', icon: Bus, color: 'text-[#8390FF]', bg: 'bg-[#263164]' },
    { title: 'Media Request', icon: Clapperboard, color: 'text-[#A78BFA]', bg: 'bg-[#39285d]' },
    { title: 'Purchase Request', icon: ShoppingCart, color: 'text-[#19D399]', bg: 'bg-[#143d3d]' },
    { title: 'Food & Refreshment Request', icon: Utensils, color: 'text-[#FB923C]', bg: 'bg-[#432a1e]' },
]
    ...
    return (
        <section className="mt-4 rounded-lg border border-[#263044] bg-[#151d2d] px-4 py-3">
            ...
            <div className="mt-1 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
                {actions.map(({ title, icon: Icon, color, bg }) => (
                    <button
                        key={title}
