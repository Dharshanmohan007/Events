...
// export default function MediaForm({ nextStep, prevStep, registerChildNavigation, eventDays = [], eventId }) {
...
//     }
//   }, [currentDay, currentDayIndex, completedDays, isLastDay, mediaData, eventId, nextStep]);
...
export default function MediaForm({ nextStep, prevStep, registerChildNavigation, eventDays = [], eventId }) {
  ...
  const handleNext = useCallback(async () => {
    ...
    if (isLastDay) {
      ...
      try {
        const payload = buildMediaPayload(mediaData);
        const formData = new FormData();
        formData.append('mediaRequirementDetails', JSON.stringify({ mediaRequirements: payload.mediaRequirements }));
  ...
  // ── Register stable nav handles — avoids infinite loop ─────────────────────
  // Always keep a ref to the latest callbacks so the registration effect
  // only needs to run once (on mount) while still calling fresh functions.
  ...
  }, [registerChildNavigation]); // run once per mount

  // Sync isLoading separately so the parent button shows "Saving..." correctly
  useEffect(() => {
    if (!registerChildNavigation) return;
