...
export function buildMediaFiles(mediaData) {
  const files = {};
  mediaData.forEach((day) => {
    if (day.poster?.referencePoster)      files["referencePosterFiles"]      = day.poster.referencePoster;
    if (day.poster?.referenceCertificate) files["referenceCertificateFiles"] = day.poster.referenceCertificate;
    if (day.video?.referenceVideo)        files["referenceFiles"]             = day.video.referenceVideo;
  });
  return files;
}
...
export default function MediaForm({
  ...
  mediaData: externalMediaData,
  onMediaDataChange,
  // onSave is intentionally NOT used — MediaForm owns its own API call.
  // This prevents the parent's isAllDaysCompleted() / Submit logic from
  // firing before all days are filled.
  errors: externalErrors = {},
}) {
  ...
  // Always-fresh refs — callbacks read from these, never from closed-over state
  ...
  // Resize array when dayCount changes (1 to 100+ days)
  useEffect(() => {
  ...
  // ── handleNext ─────────────────────────────────────────────────────────────
  // Empty deps — reads everything from refs so it is always fresh
  const handleNext = useCallback(async () => {
    ...
    // Mark current day completed immediately
    setCompletedDays((prev) => prev.includes(idx) ? prev : [...prev, idx]);

    if (!isLast) {
      // Just move to next day — do NOT call nextStep here
      setErrors((prev) => ({ ...prev, [idx + 1]: {} }));
      setCurrentDayIndex(idx + 1);
      return;
    }

    // Last day — save to API then advance parent step
    setIsLoading(true);
    setApiError("");
    try {
      const payload  = buildMediaPayload(latestData);
      const response = await fetch(`${BASE_URL}/api/events/${eventIdRef.current || ""}`, {
        method: "PUT",
        headers: {
      ...
      nextStepRef.current(); // advance parent form step
    } catch (err) {
  ...
  // navRef updated every render; proxies registered once delegate to latest.
