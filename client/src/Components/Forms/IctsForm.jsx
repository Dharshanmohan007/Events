...
//   // specialRequirements is optional but field must exist
//   return e;
// }
...
//       {/* Row 2: Expected Internet Users + Proctoring Users */}
//       <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
//         <div>
//           <CustomInput
//             labelBg="#1E1E35"
//             label="Expected Internet Users (if needed give count) *"
//             type="number"
...
//           <CustomInput
//             labelBg="#1E1E35"
//             label="Proctoring Users (if needed give count) *"
//             type="number"
...
//       {/* Row 3: Guest WiFi Needed */}
//       <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
//         <div>
//           <CustomSelect
//             labelBg="#1E1E35"
//             label="Guest WIFI Needed *"
//             value={data.guestWifi || ""}
//             onChange={update("guestWifi")}
//             options={["Yes", "No"]}
//           />
//           {errors.guestWifi && (
...
//         {/* If guest WiFi = Yes, show exceed 5 field */}
//         {data.guestWifi === "Yes" && (
//           <div>
//             <CustomSelect
//               labelBg="#1E1E35"
...
//               onChange={update("guestWifiExceed5")}
//               options={["Yes", "No"]}
//             />
...
//       {/* Row 4: Total Number of Guest Count */}
//       <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
//         <div>
//           <CustomInput
//             labelBg="#1E1E35"
...
// export default function IctsForm({ nextStep, prevStep, eventDays = [], venueData = [] }) {
//   const [currentDayIndex, setCurrentDayIndex] = useState(0);
//   const [completedDays, setCompletedDays] = useState([]);
//   const [errors, setErrors] = useState({});

//   // ictsData[dayIndex] = { [venueName]: { ...fields } }
//   const [ictsData, setIctsData] = useState({});

//   // Get selected venues for this day from venueData
//   const getVenuesForDay = (dayIndex) => {
//     const dayVenue = venueData[dayIndex];
//     if (!dayVenue) return [];
//     return dayVenue.selectedVenues || [];
//   };

//   const currentVenues = getVenuesForDay(currentDayIndex);

//   const getCardData = (dayIndex, venueName) => {
//     return ictsData[dayIndex]?.[venueName] || {};
//   };
...
//         dayErrors[venueName] = cardErrors;
//       }
//     });
//     return dayErrors;
//   };
...
//       {/* Day Timeline — same as VenueForm */}
//       <DayTimeline
//         days={eventDays}
...
//             Please go back and select venues first.
//           </p>
//         </div>
//       ) : (
...
//               onChange={(updated) =>
//                 updateCardData(currentDayIndex, venueName, updated)
//               }
...
function IctsVenueCard({ venueName, index, data, onChange, errors = {} }) {
  ...
  return (
    <div className="rounded-xl border border-[#3A3A5A] bg-[#1E1E35] p-4 sm:p-6 flex flex-col gap-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <CustomSelect labelBg="#1E1E35" label="Desktop / Laptop *" value={data.desktopLaptop || ""} onChange={update("desktopLaptop")} options={["Yes", "No"]} />
          {errors.desktopLaptop && <p className="text-red-400 text-xs mt-1">{errors.desktopLaptop}</p>}
        </div>
        <div>
          <CustomSelect labelBg="#1E1E35" label="Internet Facility *" value={data.internetFacility || ""} onChange={update("internetFacility")} options={INTERNET_FACILITY_OPTIONS} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <CustomInput labelBg="#1E1E35" label="Expected Internet Users (if needed give count) *" type="number" value={data.expectedInternetUsers || ""} onChange={updateInput("expectedInternetUsers")} />
          {errors.expectedInternetUsers && <p className="text-red-400 text-xs mt-1">{errors.expectedInternetUsers}</p>}
        </div>
        <div>
          <CustomInput labelBg="#1E1E35" label="Proctoring Users (if needed give count) *" type="number" value={data.proctorUsers || ""} onChange={updateInput("proctorUsers")} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <CustomSelect labelBg="#1E1E35" label="Guest WIFI Needed *" value={data.guestWifi || ""} onChange={update("guestWifi")} options={["Yes", "No"]} />
          {errors.guestWifi && <p className="text-red-400 text-xs mt-1">{errors.guestWifi}</p>}
        </div>
        {data.guestWifi === "Yes" && (
          <div>
            <CustomSelect labelBg="#1E1E35" label="If guest Wi-Fi Users Exceed 5 *" value={data.guestWifiExceed5 || ""} onChange={update("guestWifiExceed5")} options={["Yes", "No"]} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <CustomInput labelBg="#1E1E35" label="Total Number of Guest Count *" type="number" value={data.totalGuestCount || ""} onChange={updateInput("totalGuestCount")} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <RequirementsSelect label="Requirements *" selected={data.requirements || []} onChange={(val) => onChange({ ...data, requirements: val })} error={errors.requirements} />
        </div>
        <div>
          <CustomInput labelBg="#1E1E35" label="Others (if applicable)" value={data.others || ""} onChange={updateInput("others")} />
        </div>
      </div>
...
export default function IctsForm({ nextStep, prevStep, eventDays = [], venueData = [], eventId }) {
  const [currentDayIndex, setCurrentDayIndex] = useState(0);
  const [completedDays, setCompletedDays] = useState([]);
  const [errors, setErrors] = useState({});
