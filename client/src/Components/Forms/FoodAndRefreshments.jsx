...
function validateFoodForms(forms) {
  ...
  const errors = forms.map((form) => {
    ...
    ["Breakfast", "Lunch", "Dinner"].forEach((meal) => {
      if (form.foodTypes?.includes(meal)) {
...
export default function FoodAndRefreshments({
  ...
  //         so new containers are only added when the user clicks "Add" manually.
  const [forms, setForms] = useState(() => {
    if (initialFoodData && initialFoodData.length > 0) {
      ...
      if (savedForms.length > 0) return savedForms;
    }
    return [createForm()];
  });
  ...
  const getMealError = (id, meal, field) => {
    if (!Array.isArray(errors)) return "";
