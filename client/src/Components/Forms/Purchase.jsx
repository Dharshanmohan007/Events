...
//       giftItems: [
//         ...(personData.giftType?.includes("Trophy") ? [{
//           type: "Trophy",
...
//         }] : []),
//         ...(personData.giftType?.includes("Cash Prize") ? [{
//           type: "Cash Prize",
//           amount: parseInt(personData.cashPrizeAmount) || 0,
//         }] : []),
//         ...(personData.giftType?.includes("Voucher") ? [{
//           type: "Voucher",
//           worth: personData.voucherWorth || "",
//         }] : []),
//       ],
//     });
...
//         <span className="absolute left-3 -top-[9px] text-xs text-white px-1 z-10 pointer-events-none" style={{ backgroundColor: labelBg }}>
//           {label}
//         </span>
...
//         <div>
//           <MultiSelect labelBg="#1E1E35" label="Gift Type *" options={GIFT_TYPE_OPTIONS} selected={data.giftType || []}
//             onChange={(val) => {
...
//         </div>
//         <div>
//           <CustomSelect labelBg="#1E1E35" label="Registration Kit Needed *" value={data.registrationKitNeeded || ""}
...
//       {hasTrophy && (
//         <div>
//           <TrophyTypeSelect label="Type of Trophy Wanted *" selected={data.trophyType || []}
//             onChange={(val) => {
...
//           {showBasic && (
//             <div>
//               <CustomInput labelBg="#1E1E35" label="Basic Trophy Quantity *" type="number" value={data.basicTrophyQty || ""} onChange={updateInput("basicTrophyQty")} />
//               <ErrorMsg msg={errors.basicTrophyQty} />
//             </div>
//           )}
//           {showElite && (
//             <div>
//               <CustomInput labelBg="#1E1E35" label="Elite Trophy Quantity *" type="number" value={data.eliteTrophyQty || ""} onChange={updateInput("eliteTrophyQty")} />
...
//         <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
//           <div>
//             <CustomInput labelBg="#1E1E35" label="Cash Prize Amount *" type="number" value={data.cashPrizeAmount || ""} onChange={updateInput("cashPrizeAmount")} />
//             <ErrorMsg msg={errors.cashPrizeAmount} />
//           </div>
//           {hasVoucher && (
//             <div>
//               <CustomSelect labelBg="#1E1E35" label="Voucher worth *" value={data.voucherWorth || ""} onChange={update("voucherWorth")} options={VOUCHER_WORTH_OPTIONS} />
...
//         <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
//           <div>
//             <CustomInput labelBg="#1E1E35" label="Registration Kit Quantity *" type="number" value={data.registrationKitQty || ""} onChange={updateInput("registrationKitQty")} />
...
//   const e = {};
//   if (!data.requirementNeeded || data.requirementNeeded.length === 0) e.requirementNeeded = "Select at least one requirement";
//   if (data.requirementNeeded?.includes("Id Card") && !data.idCardQty?.trim()) e.idCardQty = "ID Card quantity is required";
//   if (data.requirementNeeded?.includes("Certificate") && !data.certificateQty?.trim()) e.certificateQty = "Certificate quantity is required";
//   if (!data.selectedPersons) e.selectedPersons = "Please select required persons";
...
// export default function Purchase({ nextStep, prevStep, registerChildNavigation, eventDays = [], eventId }) {
...
//     eventDays.map(() => ({
//       requirementNeeded: [], idCardQty: "", certificateQty: "", selectedPersons: "",
//       studentData: { giftType: [], registrationKitNeeded: "", trophyType: [], basicTrophyQty: "", eliteTrophyQty: "", cashPrizeAmount: "", voucherWorth: "", registrationKitQty: "", specialRequirements: "" },
//       guestData: { giftType: [], registrationKitNeeded: "", trophyType: [], basicTrophyQty: "", eliteTrophyQty: "", cashPrizeAmount: "", voucherWorth: "", registrationKitQty: "", specialRequirements: "" },
//     }))
//   );
...
//       setApiError("");
//       try {
//         const payload = buildPurchasePayload(dayData);
//         const id = eventId || '';
...
//     }
//   }, [current, currentDayIndex, completedDays, isLastDay, dayData, eventId, nextStep]);
...
//   }, [handleNext, handleBack, isLoading, registerChildNavigation]);
...
//       <div>
//         <MultiSelect label="Requirement Needed *" options={REQUIREMENT_OPTIONS} selected={current.requirementNeeded || []}
//           onChange={(val) => {
...
//           {showIdCard && (
//             <div>
//               <CustomInput label="Id Card Hard Copy Quantity *" type="number" value={current.idCardQty || ""} onChange={(e) => updateCurrent({ idCardQty: e.target.value })} />
//               <ErrorMsg msg={currentErrors.idCardQty} />
//             </div>
//           )}
//           {showCertificate && (
//             <div>
//               <CustomInput label="Certificate Hard Copy Quantity *" type="number" value={current.certificateQty || ""} onChange={(e) => updateCurrent({ certificateQty: e.target.value })} />
...
//       <div>
//         <CustomSelect label="Select Required Persons *" value={current.selectedPersons || ""}
...
//       {showStudent && (
//         <PersonCard title="Students" data={current.studentData || {}} onChange={(updated) => updateCurrent({ studentData: updated })} errors={currentErrors.studentData || {}} />
//       )}
//       {showGuest && (
//         <PersonCard title="Guest" data={current.guestData || {}} onChange={(updated) => updateCurrent({ guestData: updated })} errors={currentErrors.guestData || {}} />
//       )}
...
export default function Purchase({ nextStep, prevStep, registerChildNavigation, eventDays = [], eventId }) {
  ...
  const [dayData, setDayData] = useState(() => eventDays.map(() => createEmptyPurchaseDay()));

  useEffect(() => {
    setDayData((prev) => {
      const next = eventDays.map((_, index) => prev[index] || createEmptyPurchaseDay());
      return next;
    });
  }, [eventDays.length]);

  useEffect(() => {
    if (currentDayIndex >= eventDays.length && eventDays.length > 0) {
      setCurrentDayIndex(0);
    }
  }, [eventDays.length, currentDayIndex]);
  ...
  const updateCurrent = (patch) => {
    console.log('📝 Purchase updateCurrent', { currentDayIndex, patch });
    setDayData((prev) => {
      const updated = [...prev];
      updated[currentDayIndex] = { ...updated[currentDayIndex], ...patch };
      console.log('📝 Purchase updated dayData', updated[currentDayIndex]);
      return updated;
    });
    setErrors((prev) => ({ ...prev, [currentDayIndex]: {} }));
  };
  ...
  const handleNext = useCallback(async () => {
    console.log('🔍 Purchase submit current data', { currentDayIndex, current });
    const dayErrors = validateDay(current);
    console.log('🔍 Purchase submit validation', { dayErrors });
    const hasErrors = Object.keys(dayErrors).length > 0;
    setErrors((prev) => ({ ...prev, [currentDayIndex]: dayErrors }));
    if (hasErrors) return;
    ...
    if (isLastDay) {
      ...
      setApiError("");
      try {
        // Use the properly structured payload builder
        const payload = buildPurchasePayload(dayData);
        const id = eventId || '';
        const response = await fetch(`${BASE_URL}/api/events/${id}`, {
          method: 'PUT',
  ...
  // ── Register stable nav handles ─────────────────────────────────────────────
  // Store latest callbacks in a ref so the effect dependency never changes
  ...
  useEffect(() => {
    ...
    return () => registerChildNavigation({ next: null, prev: null, isLoading: false });
    // Run once on mount only — navRef.current is always fresh
  ...
  return (
    <div className="flex flex-col gap-6 pb-6">
      ...
      <h2 className="text-white text-lg font-bold">
        Purchase Details{eventDays.length > 1 ? ` – Day ${currentDayIndex + 1}` : ""}
      </h2>
