...
export default function PickupForm( nextStep, handlePrevious) {
  ...
  const addCheckpoint = (formIndex) => {
    const updated = [...forms];
    updated[formIndex].checkpoints.push({ name: "" });
    setForms(updated);
  };
  ...
  const handleCheckpointChange = (formIndex, cpIndex, value) => {
    const updated = [...forms];
    updated[formIndex].checkpoints[cpIndex].name = value;
    setForms(updated);
  };
  ...
  const removeCheckpoint = (formIndex, cpIndex) => {
    const updated = [...forms];
    updated[formIndex].checkpoints.splice(cpIndex, 1);
    setForms(updated);
  };
  ...
  return (
    <div className="w-full">
      ...
      {forms.map((form, formIndex) => (
        <div
          key={formIndex}
          className="relative bg-[#1f1f3a] p-6 rounded-xl w-full text-white mb-6"
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <CustomSelect
              ...
              onChange={(val) =>
                handleChange(formIndex, "vistaTransport", val)
              }
          ...
          <div className="relative mb-6 mt-4">
            <textarea
              className="w-full p-4 rounded-lg border border-gray-700 text-white bg-transparent"
              rows={3}
            />
