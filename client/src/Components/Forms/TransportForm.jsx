...
function VehicleMultiSelect({ label, vehicleInventory, selected = [], onToggle, loading = false }) {
  const [open, setOpen] = useState(false);
  ...
  return (
    <div ref={ref} className="relative w-full">
      ...
      >
        <span className={`text-sm truncate ${!displayText ? "text-gray-500" : "text-white"}`}>
      ...
      {open && (
        ...
        >
          ...
          {/* List */}
          <div className="max-h-52 overflow-y-auto" style={{ scrollbarWidth: "thin", scrollbarColor: "#4a4a6a transparent" }}>
            {loading ? (
              <div className="px-4 py-3 text-sm text-gray-400">Loading vehicles...</div>
            ) : filtered.length === 0 ? (
              <div className="px-4 py-3 text-sm text-gray-400">No vehicles found</div>
            ) : (
              filtered.map((vehicle) => {
                const isSelected = selected.includes(vehicle.vehicleType);
                const avail = vehicle.availableCount;
                const availLabel = avail === null ? "Outsource" : `Availability : ${avail}`;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                      <span className="text-sm text-gray-400">{availLabel}</span>
                      {isSelected && (
...
function VehicleCountInputs({ selectedVehicles, vehicleCounts, onChange, cardBg, vehicleInventory }) {
  const rows = [];
  for (let i = 0; i < selectedVehicles.length; i += 2) rows.push(selectedVehicles.slice(i, i + 2));

  // Build a lookup: vehicleType → availableCount
  const availMap = {};
  vehicleInventory.forEach((v) => { availMap[v.vehicleType] = v.availableCount; });
              ...
              <div key={vehicleType}>
                ...
                {showWarning && (
                  <p className="text-orange-400 text-xs mt-1 flex items-center gap-1">
                    ...
                    Only {available} {vehicleType}{available === 1 ? "" : "s"} available
                  </p>
                )}
              </div>
            );
...
export default function TransportForm({
  ...
  const fetchVehicleInventory = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/transport-inventory`,
        { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } }
      );
      if (!response.ok) throw new Error("Failed to fetch transport inventory");
      const data = await response.json();
  ...
  useEffect(() => { fetchVehicleInventory(); }, [fetchVehicleInventory]);
  ...
  const handleVehicleToggle = (formIndex, vehicleType) => {
    setForms((prev) => {
      ...
      const newTransport = exists ? current.filter((v) => v !== vehicleType) : [...current, vehicleType];
      const newVehicleCounts = { ...updated[formIndex].vehicleCounts };
  ...
  const requestRemoveTransport = (formIndex) => { setPendingDelete({ type: "transport", formIndex }); };
  const requestRemoveStaffMember = (formIndex, staffIndex) => { setPendingDelete({ type: "staff", formIndex, staffIndex }); };
  ...
  const handleConfirmDelete = () => {
    ...
    if (pendingDelete.type === "transport") {
      ...
      setForms((prev) => {
        ...
        const members = [...(updated[formIndex].staffMembers || [])];
        members.splice(staffIndex, 1);
        updated[formIndex] = { ...updated[formIndex], staffMembers: members, staffCount: String(members.length) };
        return updated;
      });
    }
    setPendingDelete(null);
  ...
  return (
    <div className="w-full">
      ...
      {forms.map((form, formIndex) => {
        const CARD_BG = "bg-[#1f1f3a]";
        const STAFF_BG = "bg-[#282846]";
        const resolvedInventory = getResolvedVehicleInventory();
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  <VehicleMultiSelect
                    ...
                    onToggle={(vehicleType) => handleVehicleToggle(formIndex, vehicleType)}
              ...
              {(form.vistaTransport || []).length > 0 && (
                <VehicleCountInputs
                  ...
                  onChange={(type, val) => handleVehicleCountChange(formIndex, type, val)}
                  cardBg={CARD_BG}
                  vehicleInventory={resolvedInventory}
                />
              )}
              ...
              <div className="mb-4">
                <FloatingInput
                  ...
                  onChange={(e) => handleStaffCountChange(formIndex, e.target.value)}
                  bgClass={CARD_BG}
                />
                    ...
                    <div key={si} className="relative rounded-xl p-4" style={{ backgroundColor: "#282846" }}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <FloatingInput
                          ...
                          value={staff.name}
                          onChange={(e) => handleStaffMemberChange(formIndex, si, "name", e.target.value)}
                          bgClass={STAFF_BG}
                        />
                        <FloatingInput
                          ...
                          type="number"
                          value={staff.mobile}
                          onChange={(e) => handleStaffMemberChange(formIndex, si, "mobile", e.target.value)}
                          bgClass={STAFF_BG}
                        />
                      </div>
                    </div>
