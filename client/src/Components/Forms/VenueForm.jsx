...
function MultiVenueSelect({ label, options, selected, onChange, error, totalParticipants }) {
  ...
  const toggle = (venue) => {
    ...
    // Allow only if combined capacity is enough
    if (
      totalSelectedCapacity !== Number.MAX_SAFE_INTEGER &&
      totalSelectedCapacity < totalParticipants
    ) {
      setCapacityError(
        `Selected venues have only ${totalSelectedCapacity} seats. Please select more venue(s) for ${totalParticipants} participants.`
      );
    }
  ...
  useEffect(() => {
    const fetchVenues = async () => {
      ...
      try {
        ...
        setVenuesFetchError(err.message || "Failed to load venues. Please refresh.");
      } finally {
